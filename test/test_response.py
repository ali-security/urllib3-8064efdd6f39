...
class TestBytesQueueBuffer(object):
    ...
    # Upstream bounds this at 12.5 MB with pytest-memray, asserting that we're not
    # doubling memory usage. pytest-memray is not a dependency of this branch, so the
    # mark would be inert (silently enforcing nothing) and is deliberately omitted --
    # the assertion below still covers correctness of the drain.
    ...
    # Upstream bounds this at 10.01 MB with pytest-memray; that dependency is absent
    # here, so the mark is omitted rather than left inert. The `is chunk` identity
    # assertion below is what actually proves no copy was made.
...
if not hasattr(gzip, "compress"):
    # Python 2.7 doesn't provide the `gzip.compress` shortcut.
...
class TestResponse(object):
    ...
    #   - zstd: 4.5 MiB
    # Upstream bounds this at 10 MB with pytest-memray; that dependency is absent here,
    # so the mark is omitted rather than left inert. The `_decoded_buffer` assertions in
    # the body are what actually prove the decode stayed bounded.
    ...
    # 8 MiB of zeroes compresses down to a handful of kilobytes, i.e. an
    # amplification factor of roughly 1000x. 8 MiB (rather than something
    # larger) keeps the test friendly to the most memory-constrained CI leg.
    _BOMB_SIZE = 8 * 2**20
    _BOMB_AMT = 64 * 1024
    ...
    def test_decompression_bomb_is_bounded(self, surface, data):
        """
        A tiny compressed body that expands into a much bigger payload must
        never be decoded in one go: each ``read``/``stream``/``read_chunked``
        call has to return at most ``amt`` *decoded* bytes, no matter how much
        the decoder is able to produce from the bytes read off the wire.
        """
        name, compress_func = data
        original_data = b"\0" * self._BOMB_SIZE
        compressed_data = compress_func(original_data)
        amt = self._BOMB_AMT
        # The complete bomb is delivered by a single socket read, so an
        # unbounded decoder would hand back all `_BOMB_SIZE` bytes at once.
        ...
        if surface in ("chunked_stream", "read_chunked"):
            ...
            if surface == "stream":
                ...
                chunks = iter(lambda: r.read(amt, decode_content=True), b"")

        # Bound the iteration count so a regression fails loudly instead of
        # spinning (or allocating) forever.
        max_chunks = self._BOMB_SIZE // amt + 100
        received = []
        total = 0
    ...
    # Upstream bounds this at 25 MB with pytest-memray. That dependency is absent here,
    # so instead of leaving an inert mark we assert structurally that no duplicate copy
    # of the payload is retained in the decoded buffer once the read completes.
    ...
    # Upstream bounds this at 10.5 MB with pytest-memray. That dependency is absent
    # here, so instead of leaving an inert mark we assert structurally that the
    # "do not waste memory on buffer when not decoding" path never buffers at all.
